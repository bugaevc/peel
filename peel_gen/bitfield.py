...
class Bitfield(DefinedType):
    ...
    def generate(self):
        ...
        l.extend([
            '}}; /* bitfield {} */'.format(self.emit_name),
            '',
            'static constexpr inline {}'.format(self.emit_def_name),
            'operator | ({} lhs, {} rhs)'.format(self.emit_def_name, self.emit_def_name),
            '{',
            '  return {} (static_cast<::{}> (lhs) | static_cast<::{}> (rhs));'.format(self.emit_def_name, self.c_type, self.c_type),
            '}',
            '',
            'static constexpr inline {}'.format(self.emit_def_name),
            'operator & ({} lhs, {} rhs)'.format(self.emit_def_name, self.emit_def_name),
            ...
            '',
            'static constexpr inline {}'.format(self.emit_def_name),
            'operator ~ ({} lhs)'.format(self.emit_def_name),
            '{',
            '  return {} (~static_cast<::{}> (lhs));'.format(self.emit_def_name, self.c_type),
            '}',
            '',
            'static inline {} &'.format(self.emit_def_name),
            'operator |= ({} &lhs, {} rhs)'.format(self.emit_def_name, self.emit_def_name),
            '{',
            '  lhs = lhs | rhs;',
            '  return lhs;',
            ...
            'operator ! ({} lhs)'.format(self.emit_def_name),
            '{',
            '  return !static_cast<::{}> (lhs);'.format(self.c_type),
            '}',
            '',
            'static constexpr inline bool',
            'operator + ({} lhs)'.format(self.emit_def_name),
...
class BitfieldMember(NodeHandler):
