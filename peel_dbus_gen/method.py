...
class Method:
    ...
    def generate_default_vfuncs(self):
        ...
        return '\n'.join([
            ...
            '  self->{}_async ({}[{}error, &completed, &result] (::peel::GObject::Object *_peel_source_object, ::peel::Gio::AsyncResult *_peel_res)'.format(self.cpp_name, self.in_args_forward, self.out_args_forward),
            '    {',
            '      result = _peel_source_object->cast<{}> ()->{}_finish (_peel_res, {}error);'.format(self.iface.emit_name, self.cpp_name, self.out_args_forward),
            '      completed = true;',
