...
class Method:
    ...
    def generate_default_vfuncs(self):
        ...
        l.extend([
            ...
            'static void',
            '{}_default_{}_async ({} *self, {}'.format(self.iface.emit_name, self.cpp_name, self.iface.emit_name, self.in_args.strip()),
            '  ::GAsyncReadyCallback callback, gpointer user_data, ::peel::Gio::Cancellable *cancellable,',
            '  ::peel::Gio::DBusCallFlags bus_call_flags, int bus_call_timeout_msec)',
            '{',
            ...
            '  g_task_set_static_name (_peel_task, "{}::{}");'.format(''.join(ns + '::' for ns in self.iface.cpp_namespaces) + self.iface.emit_name, self.cpp_name),
            '#endif',
            '',
            '  {}::Iface *_peel_iface = self->get_interface<{}> ();'.format(self.iface.emit_name, self.iface.emit_name),
            '  if (G_UNLIKELY (_peel_iface->_peel_vfunc_{}_sync == {}_default_{}_sync))'.format(
                self.cpp_name,
                self.iface.emit_name,
                self.cpp_name,
            ),
            '    {',
            '      const char *_peel_type_name = self->get_type_name ();',
            '      g_critical ("%s of type \'%s\' does not implement %s::%s", "{}", _peel_type_name, "{}", "{}");'.format(
                self.iface.emit_name,
                self.iface.emit_name,
                self.cpp_name,
            ),
            '      g_task_return_new_error (_peel_task, G_DBUS_ERROR, G_DBUS_ERROR_NOT_SUPPORTED,',
            '                               "%s.%s not implemented on %s", "{}", "{}", _peel_type_name);'.format(
            ...
            '    }',
            '',
            '  {}{}Data *_peel_task_data = g_slice_new0 ({}{}Data);'.format(
                self.iface.emit_name,
                self.dbus_name,
                self.iface.emit_name,
                self.dbus_name,
        ...
        for arg in self.arguments:
            if arg.direction == 'in':
                l.append('  new (&_peel_task_data->{}) {} (std::move ({}));'.format(
                    arg.cpp_name,
                    arg.generate_cpp_type(bare=True),
                    arg.cpp_name,
                ))
        l.extend([
            ...
            '    {',
            '      {} *self = reinterpret_cast<{} *> (source_object);'.format(
                self.iface.emit_name,
                self.iface.emit_name,
            ),
            '      {}{}Data *task_data = reinterpret_cast<{}{}Data *> (d);'.format(
                self.iface.emit_name,
                self.dbus_name,
                self.iface.emit_name,
                self.dbus_name,
            ...
            '',
            'static bool',
            '{}_default_{}_finish ({} *self, ::peel::Gio::AsyncResult *async_result, {}'.format(
                self.iface.emit_name,
                self.cpp_name,
                self.iface.emit_name,
                self.out_args.strip(),
            ...
            '  g_return_val_if_fail (_peel_task->get_source_tag () == {}_default_{}_async, false);'.format(
                self.iface.emit_name,
                self.cpp_name,
            ),
        ])
        ...
        l.extend([
            ...
            '',
            'static bool',
            '{}_default_{}_sync ({} *self, {}'.format(
                self.iface.emit_name,
                self.cpp_name,
                self.iface.emit_name,
                (self.in_args + self.out_args).strip(),
            ),
            '  ::peel::UniquePtr<::peel::GLib::Error> *error, ::peel::Gio::Cancellable *cancellable,',
            '  ::peel::Gio::DBusCallFlags bus_call_flags, int bus_call_timeout_msec)',
            '{',
            '  {}::Iface *iface = self->get_interface<{}> ();'.format(self.iface.emit_name, self.iface.emit_name),
            '  if (G_UNLIKELY (iface->_peel_vfunc_{}_async == {}_default_{}_async))'.format(
                self.cpp_name,
                self.iface.emit_name,
                self.cpp_name,
            ),
            ...
            '      g_set_error (reinterpret_cast<::GError **> (error), G_DBUS_ERROR, G_DBUS_ERROR_NOT_SUPPORTED,',
            '                   "%s.%s not implemented on %s", "{}", "{}", _peel_type_name);'.format(
                self.iface.interface_name,
                self.dbus_name,
            ),
